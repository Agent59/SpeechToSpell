...
public class Accio extends Item implements SpellInterface {
    private static final String NAME = "Accio";
    private static final int RANGE = 25;
    private static final int CASTING_COOLDOWN = 250;
    private static final SpellType SPELLTYPE = SpellType.CHARM;

    public Accio(Settings settings) {
        super(settings);
    }
    ...
    public void execute(ServerPlayerEntity player) {
        ...
        for (Entity entity : list) {
            if (entity instanceof ItemEntity && !(entity.getId() == player.getId())) {
