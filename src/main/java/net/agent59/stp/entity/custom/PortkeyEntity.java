...
public class PortkeyEntity extends RayEntity {
    ...
    public void tick() {
        ...
        if (!this.world.isClient()) {
            ...
            boolean bl1 = world.getBlockState(this.targetBlockPos).getMaterial().blocksMovement();
            boolean bl2 = world.getBlockState(this.targetBlockPos.withY(this.targetBlockPos.getY() + 1)).getMaterial().blocksMovement();
            if (bl1 || bl2) {
                this.kill();
            }
        }
    ...
    public void readCustomDataFromNbt2(NbtCompound nbt) {
        ...
        if (nbt.contains("targetBlockPos", 9) && (nbtList = nbt.getList("targetBlockPos", 5)).size() == 3) {
            this.targetBlockPos = new BlockPos(nbtList.getFloat(0), nbtList.getFloat(1), nbtList.getFloat(2));
        }
    }
}
