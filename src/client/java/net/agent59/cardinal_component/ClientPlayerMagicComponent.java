        ...
        try {
            switch (status) {
                ...
                case PlayerMagicComponent.SyncType.HOTBAR_SLOT -> {
                    ...
                    } else Main.LOGGER.error("The slot {} is out of bounds {}, when updating the spellHotbar",
                            slot, this.spellHotbar.length - 1);
                }
                case PlayerMagicComponent.SyncType.FULL -> {
                    NbtCompound tag = buf.readNbt();
                    if (tag != null) this.readFromNbt(tag);
                    else Main.LOGGER.error("Could not read nbtCompound from packetByteBuf {}, " +
                            "", buf);
                }
                default -> Main.LOGGER.warn("Received unknown sync status {} with packet {}", status, buf);
            }
        } catch (EncoderException e) {
            Main.LOGGER.error("Could not decode networking information for status {} from packetByteBuf {}, " +
